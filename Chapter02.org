...
There has since been a great advancement in auditory periphery models, for an extensive review
see \citet{Lopez-Poveda:2005}.


The auditory nerve input model in our \CNSM   model is called the Carney \AN model, were each version is derived from the original  
model of low \CF fibres \citep{Carney:1993}.  
Motivated by observing ANF phenomenon, Carney and colleagues have made improvements to the model over the last 20 years   
...
model  \citep[version~6][]{ZilanyBruceEtAl:2009} comprises an additional power-law synapse model, with internal fractional Gaussian noise,
that enhances the temporal recovery behaviour in \ANFs. 
...
generator. 
Figure \ref{fig:ZilanyBruceFig} shows the schematics of the Zilany version of the model
the \citep{ZilanyBruceEtAl:2009,ZilanyCarney:2010}.
...
#### 
** APPENDIX Activation and Inactivation Variable Equations

\yellownote{put this in the appendix}
...
The large number of parameters in the \CNSM model creates a challenge for a
realistic optimisation. The following sections introduce the two optimisation
