...
#+begin_src emacs-lisp export: noneresults: silent
...
[[latex:progname][NEURON]]'s conductance synapse models, /ExpSyn/ and /Exp2Syn/, were used
in the \CN stellate microcircuit.  Single exponential excitatory
synapses (\tAMPA = 0.36 ms) model the experimental recordings in \VCN
neurons \citep{GardnerTrussellEtAl:1999}.  Double exponential inhibitory
synapses are used in the network from glycinergic and GABAergic
neurons. Glycinergic synapses \citep{LeaoOleskevichEtAl:2004}
(\tGlyone=0.4 ms and \tGlytwo=2.5 ms) and \GABAa synapses
\citep{AwatramaniTurecekEtAl:2005} (\tGABAone=0.7 ms and \tGABAtwo=9.0
ms) were modeled from \MNTB neurons in mature guinea pigs.  Chlorine
...
associated reversal potential. The strength of the synapses was
...
glutamatergic-\AMPA receptors with a decay time constant of $\tAMPA =
0.36$ ms \citep{Gardner:2000,GardnerTrussellEtAl:1999}, and the reversal
...
the characteristic frequency (Hz) at this location, and constants $A_0$
