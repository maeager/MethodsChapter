...
\end{equation}
where \tau_x is the time constant of /x/, $x_\inf$ is the steady-state value of /x/ (i.e.
...
\begin{eqnarray}
\IKA(t,V) &= \gKA  a^4 b c  (V - \EK) \\
a_\infty(V) &= [1 + {\rm exp}(-(V + 31)/6)]^{-1/4} \\
b_\infty(V) &= [1 + {\rm exp}((V + 66)/7)]^{-1/2} \\
c_\infty(V) &= b_\infty \\
\end{eqnarray}
\begin{eqnarray}
\tau_a(V) &= 100  [7 {\rm exp}((V + 60)/14) + 29 {\rm exp}(-(V + 60)/24)]^-1 + 0.1 \\
\tau_b(V) &= 1000  [14 {\rm exp}((V + 60)/27) + 29 {\rm exp}(-(V + 60)/24)]^-1 + 1 \\
\tau_c(V) &= 90  [1 + {\rm exp}(-(V + 66)/17)]^-1 + 10 \\
\end{eqnarray}

Low-threshold K^{+} current
\begin{eqnarray}
\IKLT(t,V) &= \gKLT  w^4 z  (V - \EK) \\
w_\infty(V) &= [1 + {\rm exp}(-(V + 48)/6)]^{-1/4} \\
z_\infty(V) &= (1 - \zeta)  [1 + {\rm exp}((V + 71)/10)]^-1 + \zeta \quad (\zeta = 0.5)\\
\end{eqnarray}
\begin{eqnarray}
\tau_w(V) &= 100  [6 {\rm exp}((V + 60)/6) + 16 {\rm exp}(-(V + 60)/45)]^-1 + 1.5\\
\tau_z(V) &= 1000  [{\rm exp}((V + 60)/20) + {\rm exp}(-(V + 60)/8)]^-1 + 50\\
\end{eqnarray}

High-threshold K^{+} current
\begin{eqnarray}
\IKHT(t,V) &= \gKHT  [\phi n^2 + (1 - \phi)p] (V - \EK) \quad (\phi = 0.85)\\   
n_\infty(V) &= [1 + {\rm exp}(-(V + 15)/5)]^{-1/2} \\
p_\infty(V) &= [1 + {\rm exp}(-(V + 23)/6)]^-1 \\
\end{eqnarray}
\begin{eqnarray}
\tau_n(V) &= 100  [11 {\rm exp}((V + 60)/24) + 21 {\rm exp}(-(V + 60)/23)]^-1 + 0.7 \\
\tau_p(V) &= 100  [4 {\rm exp}((V + 60)/32) + 5 {\rm exp}(-(V + 60)/22)]^-1 + 5 \\
\end{eqnarray}
...
\begin{eqnarray}
\INa(t,V) &= \gNa  m^3 h (V - \ENa) \\
m_\infty(V) &= [1 + {\rm exp}(-(V + 38)/7)]^{-1} \\
h_\infty(V) &= [1 + {\rm exp}((V + 65)/6)]^{-1 }\\
\end{eqnarray}
\begin{eqnarray}
\tau_m(V) &= 10 [5 {\rm exp}((V + 60)/18) + 36 {\rm exp}(-(V + 60)/25)]^{-1} + 0.04 \\
\tau_h(V) &= 100 [7 {\rm exp}((V + 60)/11) + 10 {\rm exp}(-(V + 60)/25)]^{-1} + 0.6\\
\end{eqnarray}

Hyperpolarization-activated cation current
\begin{eqnarray}
\Ih(t,V) &= \gh  r  (V - \Eh)\\
r_\infty(V) &= [1 + {\rm exp}((V + 76)/7)]^{-1}\\
\end{eqnarray}
\begin{equation}
\tau_r(V) =  10^5 [237 {\rm exp}((V + 60)/12) + 17 {\rm exp}(-(V + 60)/14)]^{-1} + 25\\
\end{equation}
...
associated reversal potential[fn:: [[latex:progname][NEURON]]'s conductance synapse model classes /ExpSyn/ and /Exp2Syn/ were used in the \CN stellate microcircuit.].
...
The tonotopic organisation of the auditory pathway (i.e. the
...
If we had enough data from all

Need actual data from multiple neurons simultaneous - spike-timing essential, AIV ideal
...
# to natural selection to search for an optimum solution.  This
...
# response properties of TS cells will be used to set the ‘fitness’
# criteria of the genetic algorithm.  The design and implementation of
# the genetic algorithm search method is discussed in the following
# sections.
