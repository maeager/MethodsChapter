...
To examine the properties of a detailed neural model of the \CN,
 a phenomenologically-realistic auditory model is required to represent
...
The \AN input model in the \CNSM\space model is called the
...
on existing experimental evidence in most mammals.  Chapter \ref{sec:Chapter3}
